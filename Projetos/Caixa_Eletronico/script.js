...
function fazer_transferencia() {
  ...
  var valorTransferencia = parseFloat(
    prompt("Qual o valor para transferência?")
  );
  if (
  ...
  extrato.push(
    `Transferência para conta ${contaDestino}: - R$${valorTransferencia.toFixed(
      2
    )}`
  );
...
function sair() {
  alert(
    `Obrigado por utilizar nossos serviços, ${nomeUsuario}, foi um prazer ter você por aqui!`
  );
  window.close();
}
